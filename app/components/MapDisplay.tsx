...
const MapDisplay = (props: DisplayProps) => {
  ...
  ): Evt is React.TouchEvent<HTMLCanvasElement> {
  ...
  function HandleUserMouseScroll(
    WheelEvt: React.WheelEvent<HTMLCanvasElement>
  ) {
  ...
  useEffect(() => {
    ...
    if (Ctx && CanvasContext) {
      ...
      UpdateMap(
        CanvasContext,
        X + (TouchX - XOffset) * ZoomCompen,
        Y + (TouchY - YOffset) * ZoomCompen,
        Math.floor(ViewportW / 2),
        Math.floor(ViewportH / 2),
        Zoom,
        Rot,
        ViewportH,
        ViewportW
  ...
  return (
    <div className="fixed top-24 left-0 w-full h-full -z-50">
      <canvas
        ref={CanvasRef}
        onMouseDown={HandleUserEvtDown}
        onMouseUp={HandleUserEvtUp}
        onMouseMove={HandleUserEvtMove}
        onTouchStart={HandleUserEvtDown}
        onTouchEnd={HandleUserEvtUp}
        onTouchMove={HandleUserEvtMove}
