...
const Sidebar = ({ children }: Readonly<{ children: React.ReactNode }>) => {
  ...
  return (
    <div className={`bg-gray-700 inline-block ${Toggle ? " w-56 h-full sm:w-96" : "w-12 h-12 sm:w-18 sm:h-18"} transition-wh overflow-y-visible overflow-x-hidden rounded-lg`}>
      <div className="w-full h-full p-1 sm:p-3 inline-block">
        ...
        <div className={`${Toggle ? "block" : "hidden"} m-4 pb-4 pl-4 pt-4 rounded-lg bg-gray-600 z-50`}>
          {children}
        </div>
      </div>
    </div>
