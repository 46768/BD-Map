...
export default function AStar(Graph: Buffer, StartingNode: AStarCoordinate, TargetNode: AStarCoordinate) {
    ...
    function Dist(StartingNode: AStarCoordinate, EndingNode: AStarCoordinate): number {
        return Math.floor(10*(Math.sqrt(((StartingNode.x-EndingNode.x)**2)+((StartingNode.y-EndingNode.y)**2))))
    }
    ...
    while (OpenArray.length !== 0 && Limit <= 100000) {
        ...
        Neighbor.forEach((Coord) => {
            ...
            if (NeighborCoord.x >= SX || NeighborCoord.y >= SY || NeighborCoord.x < 0 || NeighborCoord.y < 0) return
            console.log(`Pos: ${NeighborCoord.x},${NeighborCoord.y} Is ${Graph[2+(NeighborCoord.x+(NeighborCoord.y*SX))]}`)
...
TestBuffer.write(String.fromCharCode(5, 5, 
    1, 1, 1, 1, 1,
    1, 0, 0, 1, 1,
    1, 1, 0, 1, 1,
    1, 1, 1, 1, 1,
