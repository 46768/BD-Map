...
export function generatePathObjects(graph: GraphData, source: number, target: number): [LineData[], PointData[]] {
	...
	for (let i = 0; i < pathFound.length-1; i++) {
		lines.push([nodes[pathFound[i]], nodes[pathFound[i+1]], [23, 224, 255, 1], 2, {zLayer: 4, tag: "pathfind-obj"}]);
		points.push([nodes[pathFound[i]], 5, [23, 224, 255, 1], {zLayer: 4, tag: "pathfind-obj"}])
	}
